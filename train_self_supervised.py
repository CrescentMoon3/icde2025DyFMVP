...
from DyFMVP.data.data_loader import GraphCollator, load_jodie_data
from DyFMVP.data.graph import Graph
from DyFMVP.eval_utils import eval_edge_prediction, warmup
from DyFMVP.model.feature_getter import NumericalFeature
from DyFMVP.model.dyfmvp import DYFMVP
...
def run(rank, world_size, *, prefix,
        ...
        restart_prob, 
        recover_from, recover_step, force, warmup_steps, train_ratio
        ):
    # Get hash
    ...
    logger = get_logger(HASH if prefix == '' else prefix)
    ...
    try:
        ...
        basic_data, graphs, dls = init_data(
            ...
            restarter_type=restarter_type, hist_len=hist_len, train_ratio=train_ratio
        )
        (
        ...
        for name, param in model.named_parameters():
            ...
            if name.startswith('neural_process'):
                NP_params += [param]
            else:
                other_params += [param]

        params = [
            {"params": NP_params, "lr": 3e-5, 'weight_decay': 0.0},
            {"params": other_params, "lr": lr, 'weight_decay': 1e-6},
        ]
        ...
        for epoch in range(epoch_start, n_epochs):
            ...
            for i_batch, (src_ids, dst_ids, neg_dst_ids, ts, eids, _, comp_graph) in enumerate(it):
                ...
                if epoch <= 5:
                    for name, param in model.named_parameters():
                        if param.grad is not None and name.startswith('neural_process'):
                ...
                contrast_loss, mutual_loss, q_target, q_context = model.contrast_and_mutual_learning(
                    src_ids, dst_ids, neg_dst_ids, ts, eids, comp_graph,
                    contrast_only=(restart_prob == 0)
                )

                loss = contrast_loss + mutual_loss * 1.0
                ...
                if q_target and q_context:
                    ...
                    loss = loss + kl_loss * 50.0

                loss.backward()
                optimizer.step()

                model.neural_process.running_r = model.neural_process.running_r.detach()
            ...
            logger.info(f'Epoch mean   kl_divergence loss: {np.mean(total_kl_loss):.4f}')
...
if __name__ == '__main__':
    ...
    run(0, 1, prefix=args.prefix, root=args.root, data=args.data,
        ...
        force=args.force, warmup_steps=args.warmup, train_ratio=args.train_ratio
        )
