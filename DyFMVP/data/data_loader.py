...
from ..model.model_utils import (anonymized_reindex, select_latest_nids,
...
class GraphCollator:
    ...
    def collate_restart_data(self, nids, ts) -> RestartData:
        if (self.restarter == 'seq') or (self.restarter == 'taro'):
...
class InteractionData(Dataset):
    ...
    def twice_sample(self, train_ratio, random_seed):
        src = self.src
        dst = self.dst
        ts = self.ts
        eids = self.eids
        ...
        real_train_data = {'src': src, 'dst': dst, 'ts': ts, 'eids': eids, 'labels': labels}
        df = pd.DataFrame(real_train_data)
        sampled_df = df.sample(frac=train_ratio, random_state=random_seed).sort_index(ignore_index=True)
        
        src = sampled_df['src'].values
        dst = sampled_df['dst'].values
        ts = sampled_df['ts'].values
        eids = sampled_df['eids'].values
        labels = sampled_df['labels'].values

        ds = InteractionData(src, dst, ts, eids, labels, self.seed, self.eval, self.neg_dst)
        return ds

    def merge_sample(self, B):
        # print(type(self.src))
        # print(self.src.shape)
        # print(B.src.shape)
        src = np.concatenate((self.src,B.src))
        dst = np.concatenate((self.dst,B.dst))
        ts = np.concatenate((self.ts,B.ts))
        eids = np.concatenate((self.eids,B.eids))
...
def load_jodie_data_for_node_task(
        ...
        root='.', data_seed=2020, val_p=0.7, test_p=0.85, train_ratio=1.0
    ...
    if not use_validation:
        train_ratio_data_mask = ts <= val_time 
        val_ratio_data_mask = np.logical_and(ts <= test_time, ts > val_time)
    ...
    if (not use_validation) and (train_ratio > 0 and train_ratio < 1):
        train_ratio_data = InteractionData(
            *[x[train_ratio_data_mask] for x in [src, dst, ts, eids, labels]],
            seed=train_seed, eval=False
        )

        # random sample
        # train_ratio_data = train_ratio_data.twice_sample(train_ratio=train_ratio, random_seed=123)

        # chrono sample
        train_sample_end_id = math.ceil(len(train_ratio_data) * train_ratio)
        train_ratio_data = train_ratio_data.get_subset(0, train_sample_end_id)

        val_ratio_data = InteractionData(
            *[x[val_ratio_data_mask] for x in [src, dst, ts, eids, labels]],
            seed=train_seed, eval=False
        )

        train_final_data = train_ratio_data.merge_sample(val_ratio_data)

    if use_validation or train_ratio==1.0:
        return nfeats, efeats, full_data, train_data, val_data, test_data,
    else:
        return nfeats, efeats, full_data, train_data, val_data, test_data, train_final_data,
