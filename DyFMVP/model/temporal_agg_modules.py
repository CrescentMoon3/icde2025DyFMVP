...
class GraphEmbedding(nn.Module):
    ...
    def compute_embedding_with_computation_graph(
        ...
        computation_graph: ComputationGraph, neural_process,
        ...
        neigh_reprs = self.compute_embedding_with_computation_graph(
            ...
            # ts=neigh_ts.flatten(),
            ts=torch.repeat_interleave(ts, n_neighbors),
        ...
        if (delta_ts < 0).any().item():
            raise ValueError('t_prev_N(i) happens after t_prev_i.')

        delta_ts_reprs = self.time_encoder(delta_ts.view(-1))  # 3D
        t0_reprs = self.time_encoder(torch.zeros_like(delta_ts[:, 0]))  # 2D

        # TODO: update emb of N(i) from t'_k+ -> t-
        neigh_reprs = neural_process.nodes_np_drift_transform(neigh_reprs, neigh_ts, ts)
        neigh_reprs = neigh_reprs.view(-1, neigh_ts.shape[-1], neigh_reprs.shape[-1])
        delta_ts_reprs = delta_ts_reprs.view(neigh_reprs.shape[0],neigh_reprs.shape[1],-1)
    ...
    def compute_embedding(
        ...
        np_neigh_nids, np_neigh_eids, np_neigh_ts, *_ = self.graph.sample_temporal_neighbor(
            np_center_nids, np_ts, self.n_neighbors)
        if False:
