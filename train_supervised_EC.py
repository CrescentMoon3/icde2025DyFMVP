...
from DyFMVP.eval_utils import eval_node_classification, eval_edge_classification
from DyFMVP.model.basic_modules import MLP,EdgeClasLayer
    ...
    try:
        ...
        if use_valid or train_ratio_key==1.0:
            ...
            # pl_hard_setting_
                                            ...
                                            use_validation=use_valid, val_p=0.075, test_p=0.3, train_ratio=train_ratio_key)
        else:
            (
        ...
        # twice random sample
        # if train_ratio_key > 0 and train_ratio_key < 1:
        ...
        if use_valid and (train_ratio_key > 0 and train_ratio_key < 1):
            ...
            offline_data = train_data.get_subset(train_sample_end_id, len(train_data))
        ...
        raw_feat_getter = NumericalFeature(
            nfeats, efeats, dim=nfeats.shape[1], register_buffer=True, device=device
        )
        ...
        # paylater
        decoder = EdgeClasLayer(in_dim=encoder.nfeat_dim * 3, class_dim=1, dropout=dropout).to(device)
        ...
        for epoch in range(n_epochs):
            ...
            for i_batch, (src_ids, dst_ids, neg_dst_ids, ts, eids, labels, comp_graph) in enumerate(it):
                ...
                pred_y = decoder(input_1=h[:bs], input_2=h[bs:], input_3=batch_edge_feat)
                ...
                # it.set_description(f'Epoch: {epoch + 1}, train for the {i_batch + 1}-th batch, train loss: {loss.item()}')

            epoch_time = time.time() - start_epoch_t0

            # extra chrono_2
            if use_valid and (train_ratio_key > 0 and train_ratio_key < 1):
                _ = eval_edge_classification(encoder, decoder, offline_dl, device, raw_feat_getter)

            val_auc = eval_edge_classification(encoder, decoder, val_dl, device, raw_feat_getter)
            ...
            if use_valid:
                if (epoch + 1) % test_interval_epochs == 0:
                    ...
                    test_auc = eval_edge_classification(encoder, decoder, test_dl, device, raw_feat_getter)
