...
from DyFMVP.data.data_loader import ChunkSampler, GraphCollator, load_jodie_data
...
from DyFMVP.model.restarters import SeqRestarter, StaticRestarter, TaroRestarter
...
def init_parser():
    ...
    parser.add_argument('--upd_src', type=str, default="right", choices=["left", "right"],
                        help='update source')
    parser.add_argument('--upd_fn', type=str, default="gru", choices=["merge", "gru", "msa"],
    ...
    parser.add_argument('--restarter_type', type=str, default="seq",
                        choices=["seq", "static", "trans"], help='Restarter type')
...
def init_data(data, root, seed, rank=None, world_size=None, *,
              ...
              restarter_type, hist_len, train_ratio):
    ...
    # wiki_hard_setting
    # ) = load_jodie_data(data, train_seed=seed, root=root, val_p=0.1, test_p=0.2)

    # pl_hard_setting
    # ) = load_jodie_data(data, train_seed=seed, root=root, val_p=0.075, test_p=0.3)
    ...
    if train_ratio > 0 and train_ratio < 1:
        train_data = train_data.twice_sample(train_ratio=train_ratio, random_seed=123)
...
def init_model(nfeats, efeats, train_graph, full_graph, full_data, device,
               *,
               ...
               restarter_type, hist_len, msg_src, upd_src, msg_tsfm_type, mem_update_type, data_name):
    ...
    elif restarter_type == 'taro':
        restarter = TaroRestarter(
            raw_feat_getter=raw_feat_getter,
            graph=train_graph,
            hist_len=hist_len,
            n_head=n_heads, dropout=dropout
